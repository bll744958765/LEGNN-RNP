...
class locationencoder(nn.Module):
    ...
    def __init__(self, input_dim, num_hidden):
        ...
        self.input_projection = Linear(self.input_dim, self.num_hidden)
        self.context_projection = Linear(self.input_dim, self.num_hidden)
        self.target_projection = Linear(self.input_dim, self.num_hidden)

    def forward(self, coord_s):
        encoder_input1 = torch.tensor(coord_s)  # concat context location (x), context value (y)
        encoder_input = self.input_projection(encoder_input1)  # (bs,nc,13)--> (bs,nc,num_hidden)
        query = self.target_projection(encoder_input1)  # (bs,nt,12)--> (bs,nt,num_hidden)
        keys = self.context_projection(encoder_input1)  # (bs,nc,12)--> (bs,nc,num_hidden)
